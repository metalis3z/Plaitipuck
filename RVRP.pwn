...
public OnPlayerCommandPerformed(playerid, cmdtext[], success)
{
    ...
    if (!success) 
    {
        ...
        if (dist < 3)
        {
            SendClientMessageEx(playerid, COLOR_GREY, "ERROR: Command not found, see '/help'");
        }
        else
        {
            SendClientMessageEx(playerid, COLOR_GREY, "ERROR: Command not found, see '/help'");
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if((newkeys & KEY_JUMP) && !IsPlayerInAnyVehicle(playerid) && !PlayerData[playerid][pAduty] && !PlayerData[playerid][pInjured] && GetPlayerSpecialAction(playerid) != SPECIAL_ACTION_USEJETPACK && !Falling[playerid])
    {
		...
		if(!strcmp(animname,"JUMP_LAND") || !strcmp(animname,"FALL_LAND"))
		{
	        ...
	        SetTimerEx("PressJumpReset", 1000, false, "i", playerid); // Makes it where if they dont spam the jump key, they wont fall

	        if(PlayerPressedJump[playerid] >= 1) // change 3 to how many jump you want before they fall
	        {
	            ApplyAnimation(playerid, "PED", "BIKE_fall_off", 4.1, 0, 1, 1, 1, 0, 1); // applies the fallover animation
	            SetTimerEx("PressJump", 3000, false, "i", playerid); // Timer for how long the animation lasts
...
public OnPlayerText(playerid, text[])
{
	...
	if(PlayerData[playerid][pCallLine] != INVALID_PLAYER_ID && !PlayerData[playerid][pIncomingCall])
	{
		...
		ProxDetector(10.0, playerid, lstr, 0xFFFFFFFF, 0xC2C2C2C2, 0x80808080, 0x52525252, 0x26262626);
	...
	else
	{
		...
		ProxDetector(20.0, playerid, lstr, 0xFFFFFFFF, 0xC2C2C2C2, 0x80808080, 0x52525252, 0x26262626);
		SetPlayerChatBubble(playerid, text, COLOR_WHITE, 10.0, 3000);
