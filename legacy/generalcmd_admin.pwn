CMD:ahelp(playerid, params[])
{
	...
	if(PlayerData[playerid][pAdmin] >= 1)
	{
		SendClientMessage(playerid, COLOR_YELLOW, "Helper: {FFFFFF}/aod, /ar, /dr, /a(dmin chat), /p2p, /apm, /reports, /setvw");
	...
	{
		SendClientMessage(playerid, COLOR_YELLOW, "Admin Level 1: {FFFFFF}/getcar, /gotocar, /hbe, /afrisk, /arevive, /setweather, /setskin");
	}
	if(PlayerData[playerid][pAdmin] >= 4)
	{
		SendClientMessage(playerid, COLOR_YELLOW, "Admin Level 2: {FFFFFF}/entercar, /sethp, /flipcar, /arevive, /gotodynamic, /veh, /fixveh, /respawncar");
...
CMD:giveweap(playerid, params[])
{
	...
	if (sscanf(params, "udI(100)", userid, weaponid, ammo))
	    return SendSyntaxMessage(playerid, "/giveweap [playerid/PartOfName] [weaponid] [ammo]");
...
CMD:gotodynamic(playerid, params[])
{
    if (PlayerData[playerid][pAdmin] < 4)
	    return SendErrorMessage(playerid, "You don't have permission to use this command.");	

	if(isnull(params))
		return SendSyntaxMessage(playerid, "/gotodynamic [house/biz/dealer/door]");
...
CMD:aheal(playerid, params[])
{
	...
	new targetid;
	if(sscanf(params, "u", targetid))
	    return SendSyntaxMessage(playerid, "/aheal [playerid]");
...
CMD:p2p(playerid, params[])
{
	...
	if (sscanf(params, "uu", userid, targetid))
	    return SendSyntaxMessage(playerid, "/p2p [playerid/PartOfName] [playerid/PartOfName]");
