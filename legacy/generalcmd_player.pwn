...
CMD:mp(playerid, params[])
	return cmd_myproperties(playerid, params);
...
CMD:s(playerid, params[])
{
    ...
    if (isnull(params))
	    return SendSyntaxMessage(playerid, "/s [shout chat]");

	if(PlayerData[playerid][pAduty] == 1)
	{
		SendNearbyMessage(playerid, 30.0, COLOR_RED, "{FF0000}%s: {FFFFFF}(( %s ))", PlayerData[playerid][pUCP], params);
		return 0;
	}
	else
	{
 	    format(lstr, sizeof(lstr), "%s shouts: %s!", ReturnName(playerid), params);
	    ProxDetector(30, playerid, lstr, 0xFFFFFFFF, 0xC2C2C2C2, 0x80808080, 0x52525252, 0x26262626);
	}
	return 1;
}
...
CMD:b(playerid, params[])
{
	...
	{
 	    format(lstr, sizeof(lstr), "%s: (( %s ))", ReturnName(playerid), params);
	    ProxDetector(20, playerid, lstr, 0xFFFFFFFF, 0xC2C2C2C2, 0x80808080, 0x52525252, 0x26262626);
	}
	return 1;
}
...
CMD:l(playerid, params[])
{
    ...
    if (isnull(params))
	    return SendSyntaxMessage(playerid, "/l [low chat]");
	...
	{
 	    format(lstr, sizeof(lstr), "%s says [low]: %s", ReturnName(playerid), params);
	    ProxDetector(10, playerid, lstr, 0xFFFFFFFF, 0xC2C2C2C2, 0x80808080, 0x52525252, 0x26262626);
	}
	return 1;
}
...
CMD:c(playerid, params[])
{
    ...
    {
		SendVehicleMessage(GetPlayerVehicleID(playerid), 0xFFFFFFFF, "%s says [car]: %s", ReturnName(playerid), params);
	}
	else
	{
